...
public class FighterState extends Subject {
	...
	private double getSpeed() {
		...
		return (0.5 * (height - weight) + 5); // + 5 beceause we want to
												// mitigate negatives
	}
	...
	private void setCurrentRule(FighterState otherFighterState) {
		...
		this.currentRule = possible
				.get(randomGenerator.nextInt(possible.size()));
	}

	public void setCurrentMoveAction() {
		Random randomGenerator = new Random();
		List<MoveAction> moveActions = currentRule.getMoveActions();
		currentMoveActionType = moveActions.get(
				randomGenerator.nextInt(moveActions.size()))
				.getMoveActionType();
	}
	...
	public void setCurrentFightAction() {
		...
		currentFightActionType = fightActions.get(
				randomGenerator.nextInt(fightActions.size()))
				.getFightActionType();
	}
	...
	private boolean isRulePossible(Logical logical,
			FighterState otherFighterState) {
		if (logical instanceof LogicalClause)
			return ((LogicalClause) logical).computeClause(
					isRulePossible(((LogicalClause) logical).getLeftHandSide(),
							otherFighterState),
					isRulePossible(((LogicalClause) logical).getLeftHandSide(),
							otherFighterState));
	...
	private void doFightAction(FighterState otherFighterState) {
		...
		case punch_low:
			if (actionPerform
					&& (Math.abs(fighterX - otherFighterState.getFighterX()) <= fighter
							.getAttributePower(Attribute.punchReach)))
				otherFighterState.doDamage(
						fighter.getAttributePower(Attribute.punchPower),
						FightActionType.punch_low);
			break;
		case punch_high:
			if (actionPerform
					&& (Math.abs(fighterX - otherFighterState.getFighterX()) <= fighter
				...
				otherFighterState.doDamage(
						fighter.getAttributePower(Attribute.punchPower),
						FightActionType.punch_high);
			break;
		case kick_low:
			if (actionPerform
					&& (Math.abs(fighterX - otherFighterState.getFighterX()) <= fighter
							.getAttributePower(Attribute.kickReach)))
				otherFighterState.doDamage(
						fighter.getAttributePower(Attribute.kickPower),
						FightActionType.kick_low);
			break;
		case kick_high:
			if (actionPerform
					&& (Math.abs(fighterX - otherFighterState.getFighterX()) <= fighter
	...
	private int damageDone(FightActionType fightActionType, int damage) {
		...
		case punch_low:
			if (actionPerform && currentMoveActionType == MoveActionType.jump)
				return 0;
			if (actionPerform
					&& currentFightActionType == FightActionType.block_low)
		...
		case punch_high:
			if (actionPerform && currentMoveActionType == MoveActionType.crouch)
				return 0;
			if (actionPerform
					&& currentFightActionType == FightActionType.block_high)
				return Math.round(damage / 5);
			break;
		}
		return damage;
