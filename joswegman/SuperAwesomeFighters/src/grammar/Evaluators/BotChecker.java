...
public class BotChecker implements BotVisit {
	List<String> conditionTypes = Arrays.asList("always", "near", "far",
			"much_stronger", "stronger", "even", "weaker", "much_weaker", null);
	List<String> moveActions = Arrays.asList("walk_towards", "walk_away",
			"run_towards", "run_away", "jump", "crouch", "stand", null);
	List<String> fightActions = Arrays.asList("block_low", "block_high",
			"kick_low", "kick_high", "punch_low", "punch_high", null);
	...
	public void visit(Characteristic character) {
		List<String> charItems = Arrays.asList("punchReach", "punchPower",
				"kickReach", "kickPower");
		boolean error = false;
		if (!charItems.contains(character.getCharacterName())) {
			error = true;
			this.errorMessages.add("Invalid Character : "
					+ character.getCharacterName());
		}
		if (!(character.getCharacterValue() < 0)
				&& (character.getCharacterValue() > 10)) {
			error = true;
			this.errorMessages.add("Character value : "
					+ character.getCharacterName() + " = "
					+ String.valueOf(character.getCharacterValue())
					+ " is not within range!");
		}
		if (!error) {
			this.initMessages.add("Character : " + character.getCharacterName()
					+ " = " + String.valueOf(character.getCharacterValue()));
		}
	}
	...
	private void checkInputRule(InputRule inputrule, boolean firstrule) {
		if ((this.moveActions.contains(inputrule.getInputrule(firstrule)))
				|| (this.fightActions.contains(inputrule
						.getInputrule(firstrule)))) {
			this.initMessages.add(inputrule.getInputrule(firstrule));
		} else {
			this.errorMessages.add("Inputrule:  "
					+ inputrule.getInputrule(firstrule) + "  is not valid!");
		}
		if (inputrule.getInputrule(true).equalsIgnoreCase(
				inputrule.getInputrule(false))) {
			if (!warningMessages.contains("Both rules, "
					+ inputrule.getInputrule(true) + ", are the same!")) {
				this.warningMessages.add("Both rules, "
						+ inputrule.getInputrule(true) + ", are the same!");
			}
		}
	}
	...
	public void visit(Condition condition) {
		if (this.operatorList.contains(condition.getType())
				|| (condition.getType().contains("const"))) {
			this.initMessages.add(condition.getType());
		} else {
			this.errorMessages.add("Type: " + condition.getType()
					+ " is not a valid type");
		}
		if (this.operatorList.contains(condition.getType())) {
		...
		if (this.conditionTypes.contains(condition.getLeft())) {
			...
			this.errorMessages.add("Condition: " + condition.getLeft()
