...
public class FightSimulatorIOld{
	...
	private FightOld fight;
	
	public FightSimulatorIOld(List<Fighter> fighters){
		fight = new FightOld(fighters,FIRST_FIGHTER_INITIAL_X,SECOND_FIGHTER_INITIAL_X);
	}

	public void Simulate(){
		System.out.println("Simulating...");
		try {
			Display.setDisplayMode(new DisplayMode(FightSimulatorIOld.WINDOW_WIDTH,FightSimulatorIOld.WINDOW_HEIGHT));
	...
	public void draw(){
		...
		try{
			Moves ffmove = Moves.valueOf(firstFightersBehavior.getMove().getName().toUpperCase());
			Attacks ffattack = Attacks.valueOf(firstFightersBehavior.getAttack().getName().toUpperCase());
		...
		try{
			Moves sfmove = Moves.valueOf(secondFightersBehavior.getMove().getName().toUpperCase());
			Attacks sfattack = Attacks.valueOf(secondFightersBehavior.getAttack().getName().toUpperCase());
	...
	private void animate(Moves move, Attacks attack, FighterLocation location) {
		/*
	...
	public boolean drawMove(FighterLocation locaton, Moves move){
		switch(move) {
	...
	public void drawAttack(FighterLocation location, Attacks attack){
		switch(attack) {
	...
	private void initializeOpenGL() {
		...
		GL11.glLoadIdentity();
		GL11.glOrtho(0, FightSimulatorIOld.WINDOW_WIDTH, FightSimulatorIOld.WINDOW_HEIGHT, 0, 1, -1);
