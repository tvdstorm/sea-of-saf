...
public class View extends JPanel {
	...
	public View() {
		...
		leftPanel = new ImagePanel(
				new ImageIcon("input/images/stand_left.png").getImage());
		leftPanel.setBounds(100, 419, 125, 275);
		add(leftPanel);

		rightPanel = new ImagePanel(new ImageIcon(
				"input/images/stand_right.png").getImage());
	...
	public void updateFighterPresentation(FighterState fighterState) {
		...
		panelToUpdate.setLocation((int) fighterState.getFighterX() * 10,
				panelToUpdate.getY());

		String imageLoc = "";
		String fighting = "";
		if (fighterState.isActionPerform())
			fighting = "_"
					+ fighterState.getCurrentFightActionType().toString();
		...
		case jump:
			if (fighterState.isActionPerform())
				imageLoc = "jump" + fighting + leftOrRight;
			else
